...
namespace NodeMarkup.Manager
{
    public class Enter
    {
        ...
        public Enter(Markup markup, ushort segmentId)
        {
            ...
            CrosswalkPointsDic = points.Where(p => (p.Location & MarkupPoint.LocationType.Edge) != 0).ToDictionary(p => p.Num, p => new MarkupCrosswalkPoint(p.Num, p.SegmentLine, p.Location));
